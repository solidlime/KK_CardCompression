...
## ベンチマーク結果（2026-05-05）
テストデータ6ファイル（キャラ×2、衣装×2、スタジオシーン×2）で計測。

| 設定                  | 合計削減率  | 備考 |
|----------------------|-----------|------|
| Fast (旧デフォルト)    | 12.73%    | numFastBytes=5 |
| Maximum (新デフォルト) | 12.82%    | numFastBytes=128、+0.09pt |
| Ultra                | 12.82%    | numFastBytes=273、Maximumとほぼ同等 |
| Maximum + PNG再圧縮   | 12.90%    | +0.17pt vs Fast |
| Ultra + PNG再圧縮     | 12.90%    | Maximumと同等 |

### 考察
- **LZMA Ultra vs Maximum**: 差はほぼ0（0.00%）。コイカツデータのLZMAゲインは既にMaximumで飽和している
- **PNG再圧縮の効果**: 衣装カードで+1.5-1.8%の削減（PNG比率が高い場合有効）、キャラ・シーンでは+0.0-0.1%程度
- **Maximumが実用的な最良の設定**: Ultraは時間がかかる割に効果ゼロ
- **重いスタジオシーンは既存PNG圧縮が良好**: サムネイルが元から最適化されているため再圧縮の恩恵が少ない
